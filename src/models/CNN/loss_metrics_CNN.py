...
def cnn_loss(denoised, clean):
    """
    Calculate the training loss for the CNN denoiser.

    The CNN directly predicts the denoised image, so the loss compares the
    predicted denoised image with the clean target image.

    Args:
        denoised: output image predicted by the CNN model
